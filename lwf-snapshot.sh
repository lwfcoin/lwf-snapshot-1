...
if [ ! -f ../lwf-node/app.js ]; then
  echo "Error: No LWF installation detected. Exiting."
  exit 1
fi

if [ "\$USER" == "root" ]; then
  echo "Error: LWF should not be run be as root. Exiting."
  exit 1
fi

LWF_CONFIG=~/lwf-node/config.json
DB_NAME="$(grep "database" $LWF_CONFIG | cut -f 4 -d '"')"
DB_USER="$(grep "user" $LWF_CONFIG | cut -f 4 -d '"')"
DB_PASS="$(grep "password" $LWF_CONFIG | cut -f 4 -d '"' | head -1)"
SNAPSHOT_COUNTER=snapshot/counter.json
SNAPSHOT_LOG=snapshot/snapshot.log
if [ ! -f "snapshot/counter.json" ]; then
  mkdir -p snapshot
  sudo chmod a+x lwf-snapshot.sh
...
create_snapshot() {
  ...
  echo "--------------------------------------------------"
  echo "..."
  sudo su postgres -c "pg_dump -Ft $DB_NAME > $SNAPSHOT_DIRECTORY'lwf_db$NOW.snapshot.tar'"
...
restore_snapshot(){
  ...
  echo "--------------------------------------------------"
  SNAPSHOT_FILE=`ls -t snapshot/lwf_db* | head  -1`
...
*)
  ...
  echo "Try: bash lwf-snapshot.sh help"
  ;;
esac
